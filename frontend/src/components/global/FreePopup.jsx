  ...
  return (
    ...
    >
        ...
        duration-300 transition-all relative bg-white w-full max-w-[90%] sm:max-w-[550px] sm:h-[410px]  flex justify-stretch flex-col-reverse sm:flex-row rounded-lg shadow-lg overflow-hidden `}>
            ...
            <p id="popup-title" className="text-[26px] font-bold">I&apos;m Giving Away Free Custom WordPress Websites  - Limited Time Offer!</p>
            <Link onClick={() => {
              closePopup();
              ReactGA.event({
                category: 'wordpress-popup',
            ...
            }} to="/quote?source=free-wordpress-popup" className="text-center w-full p-3 bg-primary text-white hover:bg-x transition-colors rounded-lg">Claim Free Website Now!</Link>
            </div>
            ...
            <button onClick={() => {closePopup();}} className=" flex p-1 justify-center items-center rounded bg-secondary text-primary absolute top-3 right-3 text-xl" aria-label="Close popup"><i className='bx bx-x'></i></button>
...
<div onClick={(e) => {if (e.target == e.currentTarget) setActive(true)}}  className="fixed max-sm:top-5 left-5 max-sm:right-5 h-10 rounded p-2 bg-x text-white text-center flex items-center max-sm:opacity-95  transition-all hover:scale-[1.03] max-sm:hover:opacity-100 sm:bottom-5 sm:gap-5 cursor-pointer z-50 " role="button" tabIndex="0" aria-label="Free WordPress Site Offer">
    <span onClick={(e) => {if (e.target == e.currentTarget) setActive(true)}} className="flex-1">
        Free WordPress Site - Act Now!
    </span>

    <button onClick={() => {
      setSmallActive(false);}} className=" flex justify-center items-center rounded bg-secondary text-primary text-xl" aria-label="Close small popup"><i className='bx bx-x'></i></button>
