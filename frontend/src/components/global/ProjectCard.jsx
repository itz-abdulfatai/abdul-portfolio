...
function ProjectCard({p}) {
  ...
  return (
    <Link to={'/works/'+p.slug} className={` ${p.slug} w-full inline-flex max-w-[400px] xl:max-w-[500px] flex-col p-5  rounded-3xl aspect-square bg-x2 gap-8 group`}>
        <div className=" overflow-hidden rounded-2xl fles justify-center items-center h-80 ">

        <img src={p.images[0] || 'https://picsum.photos/200/300?random=1'} alt=""  className=" w-full group-hover:scale-110 transition-all duration-500"/>
        </div>
        <div className=" flex flex-col md:flex-row md:items-center max-md:gap-5 justify-between">
            <div className=" flex flex-col gap-3">
                ...
                <div className=" flex gap-3 flex-wrap">
                    {p.services.map((service, index) => index < 2 &&  <div key={service} className=" p-2 rounded-lg bg-[#282828]">{service}</div> )}
                </div>
            </div>
            ...
            <i className='bx bx-arrow-back rotate-[135deg] ' ></i></span>
        </div>
    </Link>
  )
}
