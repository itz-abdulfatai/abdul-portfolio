  ...
  return (
    <footer className="min-h-[50vh] max-md:py-10  padding-x flex flex-col gap-5 ">
      ...
      <div className=' ttt flex justify-between items-center mt-10'>
        <Logo/>
        <p>
        Built with <i className='bx bx-heart text-highlight'></i> by <Link className='text-highlight' to={socials[Math.floor(Math.random() * socials.length)].link} target='blank'> {name}</Link> 
        </p>
      </div>
    </footer>
  )
