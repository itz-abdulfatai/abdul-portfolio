export  function validateAddSocial(req, res, next) {
  let {data} =  req.body
    ...
    if (errors.length > 0) return res.status(400).send({ message: errors.join(', ') })
...
export  function validateAddProject(req, res, next) {
  ...
  const errors = [];
    const validFields = ['name', 'services', 'images', 'description', 'type',  'public', 'slug', 'clientInfo', 'clientInfoId', ]
    const filtered = {}
    ...
    if (!data.name || typeof data.name !== "string") {
      ...
      if (
        ...
        (data.clientInfoId !== null && typeof data.clientInfoId !== "string")
      ) {
...
export  function validateAddTestimonial(req, res, next) {
  let {data} =  req.body
    ...
    if (data.clientInfoId && (typeof data.clientInfoId !== 'string' || data.clientInfoId.trim() === '')) {
...
export  function validateAddTool(req, res, next) {
  ...
  if (data.icon && (typeof data.icon !== 'string' || data.icon.trim() === '')) {
    errors.push('Icon, if provided, must be a valid string');
  }
